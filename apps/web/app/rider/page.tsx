...
export default function RiderPage() {
  ...
  return <Guard roles={["RIDER"]}><MobileAppShell role="RIDER">
    <section className="mt-4 overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-orange-950 p-5 text-white shadow-xl">
      ...
      {!activeTrip ? <button className={`mt-5 w-full ${profile?.available ? "bg-white text-slate-950" : "bg-orange-500 text-white"}`} disabled={busy || (!profile?.available && !isReady)} onClick={toggleAvailability}>{busy ? "Actualizando…" : profile?.available ? "Pausar recepción de viajes" : isReady ? "Conectarme para trabajar" : "Completa los requisitos para conectarte"}</button> : null}
    </section>
