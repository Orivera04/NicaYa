...
export default function RiderPage() {
  ...
  return <Guard roles={["RIDER"]}><MobileAppShell role="RIDER">{!activeTrip && readiness?.dailyQuota ? <section className="mx-3 mt-3 rounded-2xl bg-slate-950 p-4 text-white shadow-lg"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold tracking-wider text-orange-300">CUPO DIARIO · {readiness.subscription?.plan || "PLAN ACTIVO"}</p><h2 className="mt-1 text-xl font-black">{readiness.dailyQuota.remaining} viajes disponibles</h2><p className="mt-1 text-sm text-slate-300">{readiness.dailyQuota.completed} de {readiness.dailyQuota.limit} completados hoy</p></div><span className="rounded-xl bg-white/10 px-3 py-2 text-sm font-bold">{readiness.dailyQuota.remaining}/{readiness.dailyQuota.limit}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-orange-400 to-amber-300" style={{ width: `${Math.max(0, Math.min(100, (readiness.dailyQuota.remaining / readiness.dailyQuota.limit) * 100))}%` }} /></div></section> : null}<div className="dispatch-flow">
