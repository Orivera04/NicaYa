...
export default function RiderPage() {
  ...
  return <Guard roles={["RIDER"]}><MobileAppShell role="RIDER"><div className="dispatch-flow">
    <section className="dispatch-map"><MapView className="dispatch-map-canvas" rider={position} origin={activeTrip ? { lat: activeTrip.originLat, lng: activeTrip.originLng } : undefined} destination={activeTrip ? { lat: activeTrip.destinationLat, lng: activeTrip.destinationLng } : undefined} focus={focus} requests={!activeTrip ? requests.map((trip) => ({ id: trip.id, lat: trip.originLat, lng: trip.originLng, title: trip.originAddress })) : []} onRequestClick={(id) => { setSelectedId(id); setShowRequests(true); }} /><button className="map-recenter dispatch-recenter" aria-label="Centrar mi ubicación" onClick={() => setFocus(position)}>◎</button><div className="dispatch-zone"><span>{zone}</span><b>{activeTrip ? "Viaje activo" : profile?.available ? "Conectado" : "Desconectado"}</b></div>{profile?.available && !activeTrip ? <button className="dispatch-available" onClick={() => setShowRequests(true)}>◉ Disponibles <b>{requests.length}</b></button> : null}</section>
