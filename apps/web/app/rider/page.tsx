...
import { mergeTrackingPoints, TRACKING_HISTORY_LIMIT, trackingPointFromEvent, type TripTrackingEvent, type TripTrackingPoint } from "@/lib/trip-tracking";

type Profile = { available: boolean };
type RequestTrip = { id: string; originAddress: string; originLat: number; originLng: number; destinationAddress: string; destinationLat: number; destinationLng: number; stops?: Array<{ lat: number; lng: number; address: string }>; notes?: string | null; estimatedPrice: string; proposedPrice?: string | null; currency: string; distanceKm: number; estimatedDurationMin: number; riderDistanceKm?: number };
type TripLocation = TripTrackingPoint;
type RecordedTripLocation = TripTrackingEvent;
type ActiveTrip = RequestTrip & { status: "ACCEPTED" | "RIDER_ON_THE_WAY" | "RIDER_ARRIVED" | "IN_PROGRESS"; client?: { name: string; phone?: string | null }; riderLat?: number | null; riderLng?: number | null; riderAccuracy?: number | null; riderHeading?: number | null; riderLocationUpdatedAt?: string | null; locations?: TripLocation[]; startLocation?: TripLocation | null };
...
const LOCATION_PUBLISH_INTERVAL_MS = 1_500;
const LOCATION_HEARTBEAT_MS = 8_000;
...
const stages: Record<ActiveTrip["status"], { title: string; detail: string; action?: string; next?: string }> = {
  ACCEPTED: { title: "Oferta aceptada", detail: "La ruta al pasajero se activará al confirmarse la aceptación.", next: "RIDER_ON_THE_WAY" },
...
const appendConfirmedLocation = (trip: ActiveTrip, location: RecordedTripLocation): ActiveTrip => {
  const locations = mergeTrackingPoints(trip.locations, [trackingPointFromEvent(location)], TRACKING_HISTORY_LIMIT);
  ...
  return { ...trip, riderLat: location.lat, riderLng: location.lng, riderAccuracy: location.accuracy, riderHeading: location.heading, riderLocationUpdatedAt: location.recordedAt, locations: nextLocations };
};
...
export default function RiderPage() {
  ...
  const [requestDecisionReady, setRequestDecisionReady] = useState(false);
  ...
  const [tripStartPoint, setTripStartPoint] = useState<{ tripId: string; point: MapPoint } | null>(null);
  ...
  const pendingTripLocation = useRef<MapPoint | null>(null);
  const tripTransitionInFlight = useRef(false);
  const requestDecisionTimer = useRef<number | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      setActiveTrip((previous) => {
        ...
        const locations = mergeTrackingPoints(previous.locations, current.locations, TRACKING_HISTORY_LIMIT);
        ...
        const serverLocationAt = current.riderLocationUpdatedAt ? Date.parse(current.riderLocationUpdatedAt) : 0;
        const latestLocationAt = latest ? Date.parse(latest.createdAt) : 0;
        return latest && latestLocationAt > serverLocationAt
          ? { ...current, locations, riderLat: latest.lat, riderLng: latest.lng, riderAccuracy: latest.accuracy, riderHeading: latest.heading, riderLocationUpdatedAt: latest.createdAt }
          : { ...current, locations };
      });
  ...
  const publishLocation = useCallback(async (next: MapPoint & { accuracy?: number; heading?: number }) => {
    ...
    if (tripId) {
      // Mientras se confirma una transición crítica (por ejemplo recoger al
      // pasajero) no compite una escritura GPS contra la misma fila del viaje.
      // Se conserva el último punto para enviarlo justo después.
      if (tripTransitionInFlight.current) {
        pendingTripLocation.current = next;
        return;
      }
      ...
      try {
        while (pendingTripLocation.current) {
          const queued = pendingTripLocation.current;
          pendingTripLocation.current = null;
          const recorded = await api<RecordedTripLocation>(`/trips/${tripId}/location`, { method: "PATCH", body: JSON.stringify(locationPayload(queued)) }).catch((error) => {
            pendingTripLocation.current = queued;
            throw error;
          });
  ...
  const refreshLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      const next = { lat: coords.latitude, lng: coords.longitude, accuracy: coords.accuracy, heading: Number.isFinite(coords.heading) ? coords.heading ?? undefined : position.heading, capturedAt: new Date().toISOString() }; setPosition(next); if (!centeredFromGps.current) { setFocus(next); centeredFromGps.current = true; }
  ...
  useEffect(() => { lastLiveLocation.current = null; pendingTripLocation.current = null; }, [activeTrip?.id]);
  useEffect(() => {
    if (!activeTrip) {
      setTripStartPoint(null);
      return;
    }
    setTripStartPoint((current) => {
      const persisted = activeTrip.startLocation || activeTrip.locations?.[0];
      if (current?.tripId === activeTrip.id) {
        if (!persisted || distanceToMeters(current.point, persisted) <= 5) return current;
        return { tripId: activeTrip.id, point: { lat: persisted.lat, lng: persisted.lng, heading: persisted.heading ?? undefined, accuracy: persisted.accuracy ?? undefined } };
      }
      return {
        tripId: activeTrip.id,
        point: persisted
          ? { lat: persisted.lat, lng: persisted.lng, heading: persisted.heading ?? undefined, accuracy: persisted.accuracy ?? undefined }
          : { ...position },
      };
    });
  }, [activeTrip?.id, activeTrip?.startLocation?.lat, activeTrip?.startLocation?.lng, activeTrip?.locations]);
  useEffect(() => {
    if (!message) return;
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(({ coords }) => {
      const next = { lat: coords.latitude, lng: coords.longitude, accuracy: coords.accuracy, heading: Number.isFinite(coords.heading) ? coords.heading ?? undefined : undefined, capturedAt: new Date().toISOString() };
      setPosition(next);
      const previous = lastLiveLocation.current;
      const now = Date.now();
      const moved = !previous || Math.hypot(next.lat - previous.lat, next.lng - previous.lng) > .000035;
  ...
  useEffect(() => {
    ...
    const updateTracking = (location: RecordedTripLocation) => {
      setActiveTrip((current) => current?.id === location.tripId ? appendConfirmedLocation(current, location) : current);
      setPosition((current) => current && Number.isFinite(location.lat) && Number.isFinite(location.lng)
        ? { ...current, lat: location.lat, lng: location.lng, accuracy: location.accuracy ?? undefined, heading: location.heading ?? undefined }
        : current);
    };
    ["trip:requested", "trip:accepted", "trip:status-updated", "trip:cancelled"].forEach((event) => socket.on(event, update));
    socket.on("trip:tracking-updated", updateTracking);
    return () => { socket.off("trip:tracking-updated", updateTracking); socket.disconnect(); };
  }, [load]);

  const selected = useMemo(() => requests.find((trip) => trip.id === selectedId) || null, [requests, selectedId]);
  const openRequest = (tripId: string) => {
    // Un toque en el mapa solamente abre el detalle. El breve seguro contra
    // eventos pointer/click consecutivos garantiza que jamás se convierta en
    // una aceptación o contraoferta involuntaria.
    if (requestDecisionTimer.current !== null) window.clearTimeout(requestDecisionTimer.current);
    setSelectedId(tripId);
    setCounterOffer("");
    setShowAcceptConfirmation(false);
    setShowRequests(true);
    setRequestDecisionReady(false);
    requestDecisionTimer.current = window.setTimeout(() => {
      setRequestDecisionReady(true);
      requestDecisionTimer.current = null;
    }, 450);
  };
  const closeRequests = () => {
    if (requestDecisionTimer.current !== null) window.clearTimeout(requestDecisionTimer.current);
  ...
  const accept = async () => {
    if (!selected || !requestDecisionReady) return setMessage("Revisa la solicitud antes de aceptar la tarifa.");
    ...
    try {
      ...
      setTripStartPoint({ tripId: accepted.id, point: { ...position } });
      setFocus(position);
  ...
  const negotiate = async () => {
    if (!selected || !requestDecisionReady || !Number(counterOffer)) return setMessage("Ingresa un monto válido para tu contraoferta.");
    setBusy(true);
    try { await api(`/trips/${selected.id}/offers`, { method: "POST", body: JSON.stringify({ amount: Number(counterOffer) }) }); setCounterOffer(""); closeRequests(); setMessage("Contraoferta enviada. Espera la respuesta del pasajero."); }
    catch (error) { setMessage((error as Error).message); }
    finally { setBusy(false); }
  };
  const moveTrip = async (requestedStatus?: unknown) => {
    ...
    try {
      ...
      setBusy(false);
      // Publica el último GPS que se haya retenido durante la transición sin
      // obligar al rider a mover nuevamente el dispositivo.
      if (pendingTripLocation.current) void publishLocation(pendingTripLocation.current).catch(() => undefined);
    }
  };
  const cancelActiveTrip = async () => {
    if (!activeTrip) return; setBusy(true);
    try { const cancelled=await api<{ status: string }>(`/trips/${activeTrip.id}/cancel`, { method: "POST", body: JSON.stringify({ reason: "Cancelado por rider" }) }); setShowCancel(false); setMessage(cancelled.status === "REQUESTED" ? "Liberaste la solicitud para que otro rider pueda atenderla." : "Viaje cancelado."); await load(); }
    catch (error) { setMessage((error as Error).message); }
    finally { setBusy(false); }
  };
  const completeActiveTrip = async () => {
    ...
    setBusy(true);
    try { await api(`/trips/${activeTrip.id}/complete-by-rider`, { method: "POST" }); setMessage("Viaje finalizado. Recibe el pago en efectivo y agradece al pasajero."); await load(); }
    catch (error) { setMessage((error as Error).message); }
    finally { setBusy(false); }
  };
  ...
  return <Guard roles={["RIDER"]}><MobileAppShell role="RIDER"><div className="dispatch-flow">
    ...
    {showRequests && <div className="flow-modal dispatch-modal" onClick={closeRequests}><section onClick={(event) => event.stopPropagation()}><div className="sheet-handle" /><p>SOLICITUDES DISPONIBLES</p>{selected ? <><h2>Viaje cercano</h2><div className="request-detail"><span>Recogida · {selected.riderDistanceKm?.toFixed(1) || "—"} km</span><b>{selected.originAddress}</b><i>↓</i><span>Destino</span><b>{selected.destinationAddress}</b><strong>{money(selected)}</strong><small>{selected.distanceKm.toFixed(1)} km · {selected.estimatedDurationMin} min</small></div><div className="counter-offer"><label>Propón otra tarifa si lo necesitas</label><div><input type="number" value={counterOffer} onChange={(event) => setCounterOffer(event.target.value)} placeholder="Monto en córdobas" /></div></div><div className="request-decision-actions"><button className="request-counter-action" disabled={busy || !requestDecisionReady || !counterOffer} onClick={() => void negotiate()}>{busy ? "Procesando…" : "Contraofertar"}</button><button className="trip-main-action request-accept-action" disabled={busy || !requestDecisionReady} onClick={() => void accept()}>{busy ? "Procesando…" : requestDecisionReady ? "Aceptar tarifa" : "Revisando solicitud…"}<span>→</span></button></div><button className="cancel-link" onClick={() => { setSelectedId(null); setRequestDecisionReady(false); }}>Volver a solicitudes</button></> : <><h2>Solicitudes cercanas</h2><div className="request-list">{requests.length ? requests.map((trip) => <button key={trip.id} onClick={() => { setFocus({ lat: trip.originLat, lng: trip.originLng }); openRequest(trip.id); }}><span>{trip.riderDistanceKm?.toFixed(1) || "—"} km</span><b>{trip.originAddress}</b><small>→ {trip.destinationAddress}</small><strong>{money(trip)}</strong></button>) : <p className="waiting-copy">No hay solicitudes en tu radio de trabajo.</p>}</div></>}</section></div>}
    {showAcceptConfirmation && selected && <div className="flow-modal dispatch-modal" onClick={() => setShowAcceptConfirmation(false)}><section className="accept-request-confirmation" onClick={(event) => event.stopPropagation()}><div className="sheet-handle" /><p>CONFIRMAR TARIFA</p><h2>¿Aceptar este viaje?</h2><span>Verifica el monto antes de confirmar. Al aceptarlo iniciarás la ruta hacia el pasajero.</span><div className="request-detail"><span>Recogida</span><b>{selected.originAddress}</b><i>↓</i><span>Destino</span><b>{selected.destinationAddress}</b><strong>{money(selected)}</strong><small>{selected.distanceKm.toFixed(1)} km · {selected.estimatedDurationMin} min</small></div><button className="trip-main-action" disabled={busy} onClick={() => void accept()}>{busy ? "Aceptando…" : `Sí, aceptar por ${money(selected)}`}<span>→</span></button><button className="cancel-link rider-modal-dismiss" disabled={busy} onClick={() => setShowAcceptConfirmation(false)}>Revisar oferta</button></section></div>}
