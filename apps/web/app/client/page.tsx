...
export default function ClientPage() {
  ...
  const load = async () => {
    try {
      ...
      if (current) { setTripId(current.id); setActiveTrip(current); setTripInfoExpanded(true); setStage(current.status === "REQUESTED" ? "SEARCHING" : "TRACKING"); }
  ...
  const reset = () => { setTripId(null); setActiveTrip(null); setTripInfoExpanded(true); setOffers([]); setDestination(null); setQuote(null); setProposedPrice(""); setStage("ROUTE"); setEditing("destination"); };
