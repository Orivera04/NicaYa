...
export default function ClientPage() {
  ...
  return <Guard roles={["CLIENT"]}><MobileAppShell role="CLIENT"><div className="trip-flow pb-4">
    ...
    {stage === "TRACKING" && activeTrip && <><section className="relative mt-3"><MapView origin={{ lat: activeTrip.originLat, lng: activeTrip.originLng }} destination={{ lat: activeTrip.destinationLat, lng: activeTrip.destinationLng }} rider={activeTrip.rider?.riderProfile?.workZoneLat != null && activeTrip.rider?.riderProfile?.workZoneLng != null ? { lat: activeTrip.rider.riderProfile.workZoneLat, lng: activeTrip.rider.riderProfile.workZoneLng } : undefined} focus={{ lat: activeTrip.originLat, lng: activeTrip.originLng }} /><span className="map-legend">♙ Pasajero&nbsp;&nbsp; 🏍 Rider&nbsp;&nbsp; ⚑ Destino</span></section><section className="tracking-sheet"><p>VIAJE ACTUAL</p><h1>{activeTrip.rider?.name || "Rider asignado"}</h1><span>{activeTrip.rider?.riderProfile?.vehicleModel || "Motocicleta confirmada"}{activeTrip.rider?.riderProfile?.vehiclePlate ? ` · ${activeTrip.rider.riderProfile.vehiclePlate}` : ""}</span><div className="trip-status"><b>{activeTrip.status.replaceAll("_", " ")}</b><span>{statusCopy[activeTrip.status] || "Actualizando el estado de tu viaje."}</span></div><div className="tracking-route"><span>{activeTrip.originAddress}</span><i>↓</i><span>{activeTrip.destinationAddress}</span></div>{activeTrip.status === "IN_PROGRESS" ? <button className="trip-main-action" disabled={busy} onClick={complete}>{busy ? "Finalizando…" : "Confirmar llegada"}<span>→</span></button> : <button className="cancel-link" onClick={() => setShowCancel(true)}>Cancelar según las reglas operativas</button>}</section></>}
