...
export function LocationSearch({ onSelect }: { onSelect: (place: GeocodedPlace) => void }) {
  ...
  const search = async (event: FormEvent) => {
    ...
    if (query.trim().length < 3) { setMessage("Escribe al menos 3 caracteres."); return; }
    setLoading(true);
    try {
      const matches = await api<GeocodedPlace[]>(`/geocoding/search?q=${encodeURIComponent(query.trim())}`);
      setResults(matches);
