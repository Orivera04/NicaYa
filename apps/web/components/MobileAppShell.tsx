...
const items: Record<Role, Item[]> = {
  CLIENT: [
    { label: "Inicio", href: "#inicio", icon: "⌂" }, { label: "Buscar viaje", href: "#viaje", icon: "⌕" }, { label: "Lugares guardados", href: "#lugares", icon: "⌖" }, { label: "Historial", href: "#historial", icon: "◷" }, { label: "Cuenta", href: "#cuenta", icon: "◉" },
  ],
  RIDER: [
    { label: "Inicio", href: "#cuenta", icon: "⌂" }, { label: "Zona de trabajo", href: "#solicitudes", icon: "⌖" }, { label: "Suscripcion", href: "#suscripcion", icon: "▣" }, { label: "Ganancias", href: "#ganancias", icon: "$" }, { label: "Historial", href: "#historial", icon: "◷" },
  ],
  ADMIN: [
    { label: "Resumen", href: "#resumen", icon: "⌂" }, { label: "Riders", href: "#riders", icon: "♞" }, { label: "Pagos", href: "#pagos", icon: "$" }, { label: "Planes", href: "#planes", icon: "▣" }, { label: "Viajes", href: "#viajes", icon: "◷" }, { label: "Anuncios", href: "#publicidad", icon: "▤" },
  ],
};
...
export function MobileAppShell({ role, children }: { role: Role; children: ReactNode }) {
  ...
  return <main className="mx-auto min-h-dvh max-w-md bg-slate-50 pb-24">
    ...
    {open ? <div className="fixed inset-0 z-[100] bg-slate-950/50" onClick={() => setOpen(false)}><aside className="h-full w-80 overflow-y-auto bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="rounded-2xl bg-slate-950 p-4 text-white"><b className="text-2xl">Moto<span className="text-orange-500">Ya</span></b><p className="mt-1 text-sm text-slate-300">{titles[role]} · Viaja rapido, seguro y cerca.</p></div><nav className="mt-5 space-y-2">{links.map((item) => <a key={item.label} className="flex items-center gap-3 rounded-xl border p-3 font-semibold" href={item.href} onClick={() => setOpen(false)}><span className="text-orange-500">{item.icon}</span>{item.label}</a>)}{role === "CLIENT" ? <Link className="flex items-center gap-3 rounded-xl border p-3 font-semibold" href="/rider" onClick={() => setOpen(false)}><span className="text-orange-500">♞</span>Quiero ser rider</Link> : null}<button className="mt-4 w-full text-left text-red-600" onClick={signOut}>Cerrar sesion</button></nav></aside></div> : null}
    <div className="px-4">{children}</div>
    <nav className="fixed bottom-0 left-1/2 z-40 flex w-full max-w-md -translate-x-1/2 justify-around border-t border-slate-200 bg-white px-1 py-2 shadow-[0_-8px_24px_rgba(15,23,42,.08)]">{links.slice(0, 5).map((item) => <a key={item.label} href={item.href} className="flex min-w-12 flex-col items-center gap-0.5 rounded-xl px-1 py-1 text-[10px] font-medium text-slate-500"><span className="text-xl text-orange-500">{item.icon}</span>{item.label}</a>)}</nav>
  </main>;
}
