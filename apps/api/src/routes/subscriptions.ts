...
subscriptionsRouter.post("/payments/:id/transfer", authorize("RIDER"), async (req, res) => { const data = z.object({ bankName: z.string().min(2).max(100), transferReference: z.string().min(3).max(100), payerName: z.string().max(100).optional(), proofReference: z.string().regex(/^data:image\/(jpeg|png|webp);base64,/).max(500000) }).parse(req.body); res.json(await submitTransfer(req.user!.id, req.params.id, data)); });
...
subscriptionsRouter.get("/admin/payments", authorize("ADMIN"), async (_req, res) => res.json(await prisma.payment.findMany({ include: { order: { include: { rider: { include: { user: { select: { id: true, name: true, email: true, phone: true, status: true } } } }, plan: true } }, method: true }, orderBy: { createdAt: "desc" }, take: 100 })));
