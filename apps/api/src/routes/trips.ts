...
tripsRouter.post("/",authorize("CLIENT"),asyncHandler(async(req,res)=>{const d=tripCreateSchema.parse(req.body);const trip=await createTrip(req.user!.id,d.origin,d.destination,d.serviceCode,d.proposedPrice,d.notes,d.stops);req.app.get("io")?.to("riders").emit("trip:requested",trip);res.status(201).json(trip);}));
...
tripsRouter.post("/:id/accept",authorize("RIDER"),async(req,res)=>{const location=tripLocationSchema.optional().parse(req.body?.location);const accepted=await acceptTrip(req.params.id,req.user!.id,location);const t=accepted.trip;const io=req.app.get("io");if(accepted.trackingLocation)emitTrackingUpdate(io,accepted.trackingLocation);io?.to(`user:${t.clientId}`).emit("trip:accepted",t);io?.to(`user:${t.riderId}`).emit("trip:accepted",t);io?.to("admins").emit("trip:status-updated",t);res.json(t);});
