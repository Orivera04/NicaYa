...
tripsRouter.patch("/:id/status",authorize("RIDER"),async(req,res)=>{const tripId=z.string().min(1).parse(req.params.id);const data=z.object({status:z.nativeEnum(TripStatus),location:tripLocationSchema.optional()}).parse(req.body);const t=data.status==="RIDER_ARRIVED"?await boardPassenger(tripId,req.user!.id,data.location):await changeStatus(tripId,req.user!.id,req.user!.role,data.status);const io=req.app.get("io");if(data.status==="RIDER_ARRIVED"&&data.location)io?.to(`user:${t.clientId}`).emit("trip:location-updated",{tripId:t.id,clientId:t.clientId,...data.location,recordedAt:t.riderLocationUpdatedAt||new Date()});io?.to(`user:${t.clientId}`).emit("trip:status-updated",t);if(t.riderId)io?.to(`user:${t.riderId}`).emit("trip:status-updated",t);io?.to("admins").emit("trip:status-updated",t);res.json(t);});
tripsRouter.patch("/:id/location",authorize("RIDER"),async(req,res)=>{const location=z.object({lat:z.number().finite().min(-90).max(90),lng:z.number().finite().min(-180).max(180),accuracy:z.number().finite().min(0).max(5000).optional(),heading:z.number().finite().min(0).max(360).optional()}).parse(req.body);const update=await recordTripLocation(req.params.id,req.user!.id,location);req.app.get("io")?.to(`user:${update.clientId}`).emit("trip:location-updated",update);res.json(update);});
...
tripsRouter.post("/:id/cancel",async(req,res)=>{const reason=z.string().trim().min(2).max(160).optional().parse(req.body?.reason);const t=await cancelTrip(req.params.id,req.user!.id,req.user!.role,reason);req.app.get("io")?.to(`user:${t.clientId}`).emit("trip:cancelled",t);if(t.riderId)req.app.get("io")?.to(`user:${t.riderId}`).emit("trip:cancelled",t);res.json(t);});
