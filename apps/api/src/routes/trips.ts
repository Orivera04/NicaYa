...
import { acceptTrip,acceptOffer,cancelTrip,changeStatus,completeTripByClient,createTrip,estimate,makeOffer,recordTripLocation,rejectOffer } from "../services/trip.service.js";
...
tripsRouter.patch("/:id/status",authorize("RIDER"),async(req,res)=>{const status=String(req.body.status) as TripStatus;const t=await changeStatus(req.params.id,req.user!.id,req.user!.role,status);const io=req.app.get("io");io?.to(`user:${t.clientId}`).emit("trip:status-updated",t);if(t.riderId)io?.to(`user:${t.riderId}`).emit("trip:status-updated",t);io?.to("admins").emit("trip:status-updated",t);res.json(t);});
