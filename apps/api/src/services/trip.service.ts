...
const pickupCodePattern=/^\d{5}$/;
const generatePickupCode=()=>randomInt(10_000,100_000).toString();
const pickupCodesMatch=(provided:string,expected:string)=>{
...
export async function boardPassenger(tripId:string,riderId:string,pickupCode:string,location?:TrackingLocationInput){
  return retrySerializableTransaction(()=>prisma.$transaction(async tx=>{
    // The rider can start a trip only with the five-digit code supplied by the
    // passenger. GPS is recorded when available, but never blocks boarding.
    ...
    if(!pickupCodePattern.test(pickupCode)||!trip.pickupCode||!pickupCodesMatch(pickupCode,trip.pickupCode)) fail(409,"INVALID_PICKUP_CODE","El código de 5 dígitos no corresponde a este viaje.");
    ...
    const updated=await tx.trip.updateMany({where:{id:tripId,riderId,status:"RIDER_ON_THE_WAY",pickupCode:trip.pickupCode},data:{status:"IN_PROGRESS",pickupCode:null}});
...
export async function createTrip(clientId:string, origin:{lat:number;lng:number;address:string}, destination:{lat:number;lng:number;address:string}, serviceCode="MOTO", proposedPrice?:number, notes?:string, stops:Array<{lat:number;lng:number;address:string;label?:string}>=[]){ const quote=await estimate(origin,destination); if(proposedPrice!==undefined&&(proposedPrice<quote.minimumFare||proposedPrice>quote.maximumFare))fail(422,"INVALID_PROPOSED_PRICE",`La propuesta debe estar entre ${quote.minimumFare} y ${quote.maximumFare} ${quote.currency}.`); return prisma.$transaction(async tx=>{ const exists=await tx.trip.findFirst({where:{clientId,status:{in:active}}}); if(exists) fail(409,"CLIENT_HAS_ACTIVE_TRIP","Ya tienes un viaje activo."); const trip=await tx.trip.create({data:{clientId,pickupCode:generatePickupCode(),originLat:origin.lat,originLng:origin.lng,originAddress:origin.address,destinationLat:destination.lat,destinationLng:destination.lng,destinationAddress:destination.address,distanceKm:quote.distanceKm,estimatedDurationMin:quote.estimatedDurationMin,estimatedPrice:new Prisma.Decimal(quote.estimatedPrice),proposedPrice:proposedPrice?new Prisma.Decimal(proposedPrice):null,serviceCode,notes:notes?.trim()||null,stops:stops.length?stops:Prisma.JsonNull,expiresAt:new Date(Date.now()+5*60_000),currency:quote.currency}}); await tx.tripStatusHistory.create({data:{tripId:trip.id,status:"REQUESTED",actorId:clientId}}); return trip; },{isolationLevel:"Serializable"}); }
...
export async function acceptTrip(tripId:string,riderId:string,location?:TrackingLocationInput){ return retrySerializableTransaction(()=>prisma.$transaction(async tx=>{ await eligibleRider(tx,riderId); const trip=await tx.trip.findUnique({where:{id:tripId}});if(!trip||!trip.expiresAt||trip.expiresAt<new Date())fail(409,"TRIP_EXPIRED","La solicitud ya venció."); const updated=await tx.trip.updateMany({where:{id:tripId,status:"REQUESTED",riderId:null,expiresAt:{gt:new Date()}},data:{status:"RIDER_ON_THE_WAY",riderId,finalPrice:trip.proposedPrice||trip.estimatedPrice}}); if(updated.count!==1) fail(409,"TRIP_ALREADY_ACCEPTED","El viaje ya fue aceptado por otro rider."); let trackingLocation:TripTrackingUpdate|null=null; if(location){const recordedAt=normalizeCapturedAt(location.capturedAt);await tx.trip.update({where:{id:tripId},data:{riderLat:location.lat,riderLng:location.lng,riderAccuracy:location.accuracy??null,riderHeading:location.heading??null,riderLocationUpdatedAt:recordedAt}});const persisted=await tx.tripLocation.create({data:{tripId,lat:location.lat,lng:location.lng,accuracy:location.accuracy,heading:location.heading,createdAt:recordedAt}});trackingLocation=toTrackingUpdate({id:trip.id,clientId:trip.clientId,riderId},persisted);} await tx.tripOffer.updateMany({where:{tripId,status:"PENDING"},data:{status:"REJECTED"}}); await tx.tripStatusHistory.createMany({data:[{tripId,status:"ACCEPTED",actorId:riderId},{tripId,status:"RIDER_ON_THE_WAY",actorId:riderId}]}); const accepted=await tx.trip.findUniqueOrThrow({where:{id:tripId},include:{client:{select:{id:true,name:true,phone:true}},rider:{select:{id:true,name:true,phone:true,riderProfile:{select:{vehicleModel:true,vehiclePlate:true}}}}}}); return {trip:accepted,trackingLocation}; },{isolationLevel:Prisma.TransactionIsolationLevel.Serializable})); }
export async function changeStatus(tripId:string,actorId:string,role:string,next:TripStatus){ const trip=await prisma.trip.findUnique({where:{id:tripId}}); if(!trip) fail(404,"TRIP_NOT_FOUND","Viaje no encontrado."); if(role!=="RIDER"||trip.riderId!==actorId) fail(403,"FORBIDDEN","No puedes actualizar este viaje."); if(!transitions[trip.status]?.includes(next)) fail(409,"INVALID_TRIP_TRANSITION","La transición de estado no está permitida."); const boardingPassenger=trip.status==="RIDER_ON_THE_WAY"&&next==="RIDER_ARRIVED"; if(boardingPassenger) fail(409,"PICKUP_CODE_REQUIRED","Ingresa el código de 5 dígitos proporcionado por el pasajero para iniciar el viaje."); return prisma.$transaction(async tx=>{ const result=await tx.trip.updateMany({where:{id:tripId,status:trip.status,riderId:actorId},data:{status:boardingPassenger?"IN_PROGRESS":next}}); if(!result.count) fail(409,"TRIP_CHANGED","El viaje cambió, vuelve a intentarlo."); if(boardingPassenger) await tx.tripStatusHistory.createMany({data:[{tripId,status:"RIDER_ARRIVED",actorId},{tripId,status:"IN_PROGRESS",actorId}]}); else await tx.tripStatusHistory.create({data:{tripId,status:next,actorId}}); return tx.trip.findUniqueOrThrow({where:{id:tripId},include:{client:{select:{id:true,name:true,phone:true}},rider:{select:{id:true,name:true,phone:true,riderProfile:{select:{vehicleModel:true,vehiclePlate:true}}}},histories:true}}); }); }
...
export async function completeTripByRider(tripId:string,riderId:string){ return prisma.$transaction(async tx=>{ const result=await tx.trip.updateMany({where:{id:tripId,riderId,status:"IN_PROGRESS"},data:{status:"COMPLETED"}}); if(!result.count) fail(409,"TRIP_NOT_COMPLETABLE","Solo puedes finalizar un viaje propio que esté en curso."); await tx.tripStatusHistory.create({data:{tripId,status:"COMPLETED",actorId:riderId}}); await tx.auditLog.create({data:{actorId:riderId,action:"TRIP_COMPLETED_BY_RIDER",entity:"Trip",entityId:tripId}}); return tx.trip.findUniqueOrThrow({where:{id:tripId},include:{client:{select:{id:true,name:true,phone:true}},rider:{select:{id:true,name:true,phone:true,riderProfile:{select:{vehicleModel:true,vehiclePlate:true}}}}}}); },{isolationLevel:"Serializable"}); }
