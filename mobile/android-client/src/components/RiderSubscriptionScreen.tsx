...
export function RiderSubscriptionScreen({ onBack, onMessage }: { onBack: () => void; onMessage: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    const safeOrders = Array.isArray(orders) ? orders.map(order => ({ ...order, payments: Array.isArray(order.payments) ? order.payments : [] })) : [];
    setPlans(Array.isArray(nextPlans) ? nextPlans : []); setMethods(Array.isArray(nextMethods) ? nextMethods : []); setMethod(current => current || nextMethods[0] || null); setSubscription(nextSubscription); setStage(current => current === "summary" && !nextSubscription ? "plans" : current);
    setPendingOrder(safeOrders.find(order => order.status === "PENDING_PAYMENT" || order.status === "PENDING_REVIEW") || null);
  }, []);
  ...
  const sendPayment = async () => {
    ...
    try {
      ...
      setCreated(null); setPendingOrder(null); setSelected(null); setProof(""); setTransferRef(""); setStage("summary"); await load(); onMessage("Comprobante enviado a validación administrativa.");
    } catch (error) { onMessage((error as Error).message); }
    finally { setBusy(false); }
  };
