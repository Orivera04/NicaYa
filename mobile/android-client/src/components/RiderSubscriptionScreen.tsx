...
export function RiderSubscriptionScreen({ onBack, onMessage }: { onBack: () => void; onMessage: (message: string) => void }) {
  ...
  const [stage, setStage] = useState<"summary" | "plans" | "checkout">("summary");
  ...
  const canChoose = !waiting && !created && (!active || stage === "plans");
  const canCheckout = !waiting && !created && stage === "checkout" && Boolean(selected);

  const createOrder = async () => {
    if (!selected || !method) return;
    setBusy(true);
    try { const nextCreated = await api<CreatedOrder>("/subscriptions/orders", { method: "POST", body: JSON.stringify({ planId: selected.id, methodCode: method.code }) }); setPendingOrder(nextCreated.order); setCreated(nextCreated); setStage("checkout"); }
  ...
  return <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    <Pressable onPress={() => { if (stage === "checkout") { setSelected(null); setCreated(null); setStage("plans"); } else if (stage === "plans" && active) { setStage("summary"); } else { onBack(); } }} hitSlop={12}><Text style={styles.back}>← {stage === "checkout" ? "Cambiar plan" : "Volver a mi cuenta"}</Text></Pressable>
