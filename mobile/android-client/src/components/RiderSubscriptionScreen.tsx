...
export function RiderSubscriptionScreen({ onBack, onMessage }: { onBack: () => void; onMessage: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setPlans(nextPlans); setMethods(nextMethods); setMethod(current => current || nextMethods[0] || null); setSubscription(nextSubscription);
  ...
  const canChoose = !waiting && !created && (!active || renewing);

  const createOrder = async () => {
    if (!selected || !method) return;
    setBusy(true);
    try { setCreated(await api<CreatedOrder>("/subscriptions/orders", { method: "POST", body: JSON.stringify({ planId: selected.id, methodCode: method.code }) })); }
  ...
  const sendPayment = async () => {
    ...
    try {
      ...
      setCreated(null); setSelected(null); setProof(""); setTransferRef(""); setRenewing(false); await load(); onMessage("Comprobante enviado a validación administrativa.");
    } catch (error) { onMessage((error as Error).message); }
    finally { setBusy(false); }
  };
  ...
  return <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    <Pressable onPress={onBack} hitSlop={12}><Text style={styles.back}>← Volver a mi cuenta</Text></Pressable>
    <Text style={styles.eyebrow}>SUSCRIPCIÓN MOTOYA</Text><Text style={styles.title}>Mi suscripción</Text><Text style={styles.subtitle}>Elige, paga y consulta el estado de tu plan.</Text>

    {active && !renewing && !created ? <>
