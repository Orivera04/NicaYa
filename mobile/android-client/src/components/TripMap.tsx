...
export function TripMap({ trip, origin, destination, currentLocation, route = [], editable = false, onMapPress, onOriginPress, onDestinationPress, label, height = 340, fill = false, searching = false, hideLabel = false, followPoint }: Props) {
  ...
  const routeHeading = useMemo(() => {
    if (!renderedRider || planned.length < 2) return undefined;
    let nearestIndex = 0; let nearestDistance = Number.POSITIVE_INFINITY;
    planned.forEach((point, index) => { const distance = metersBetween(renderedRider, point); if (distance < nearestDistance) { nearestDistance = distance; nearestIndex = index; } });
    const nextPoint = planned.slice(nearestIndex + 1).find(point => metersBetween(renderedRider, point) >= 18);
    return nextPoint ? bearingBetween(renderedRider, nextPoint) : undefined;
  }, [renderedRider?.lat, renderedRider?.lng, planned]);
  const candidateHeading = routeHeading ?? validHeading(rider?.heading) ?? validHeading(trip?.riderHeading) ?? derivedHeading;
...
const styles = StyleSheet.create({
  ...
  pin: { minWidth: 55, alignItems: "center", justifyContent: "center", elevation: 20, zIndex: 20, position: "relative", shadowColor: "#00143F", shadowOpacity: .32, shadowRadius: 12, shadowOffset: { width: 0, height: 5 } }, pin_origin: {}, pin_destination: {}, pin_rider: { zIndex: 30 }, pin_riderStart: {}, pin_client: {}, pinPulse: { position: "absolute", top: 1, width: 48, height: 48, borderRadius: 24, borderWidth: 2 }, pinPulse_client: { borderColor: theme.violet, backgroundColor: "rgba(157,78,221,.22)" }, pinPulse_rider: { borderColor: "#176BDE", backgroundColor: "rgba(23,107,222,.20)" },
  riderPinBackdrop: { position: "absolute", top: -4, width: 62, height: 76, borderRadius: 25, backgroundColor: "#176BDE", borderWidth: 3, borderColor: theme.white }, pinIconWrap: { width: 48, height: 48, borderRadius: 19, borderWidth: 3, borderColor: theme.white, alignItems: "center", justifyContent: "center" }, pinIconWrap_origin: { backgroundColor: "#0C9B8B" }, pinIconWrap_destination: { backgroundColor: theme.orange }, pinIconWrap_rider: { backgroundColor: "#176BDE" }, pinIconWrap_riderStart: { backgroundColor: "#314265" }, pinIconWrap_client: { backgroundColor: theme.violet }, pinIcon: { color: theme.white, fontSize: 21, fontWeight: "900", lineHeight: 25 }, riderPinIcon: { fontSize: 22 },
  pinLabel: { maxWidth: 72, marginTop: -1, paddingHorizontal: 7, paddingVertical: 3, borderRadius: 9, overflow: "hidden", color: theme.white, backgroundColor: "#070B1C", fontSize: 10, fontWeight: "900", textAlign: "center" }, pinPointer: { width: 0, height: 0, marginTop: -1, borderLeftWidth: 7, borderRightWidth: 7, borderTopWidth: 8, borderLeftColor: "transparent", borderRightColor: "transparent" }, pinPointer_origin: { borderTopColor: "#0C9B8B" }, pinPointer_destination: { borderTopColor: theme.orange }, pinPointer_rider: { borderTopColor: "#176BDE" }, pinPointer_riderStart: { borderTopColor: "#314265" }, pinPointer_client: { borderTopColor: theme.violet }, pinInteractive: { transform: [{ scale: 1.05 }] }
});