...
export function TripMap({ trip, origin, destination, currentLocation, route = [], editable = false, onMapPress, onOriginPress, onDestinationPress, label, height = 340, fill = false, searching = false, hideLabel = false }: Props) {
  ...
  // A dashed guide connects rider and pickup; the drivable route remains blue and white.
  const pickupGuide = useMemo(() => rider && tripOrigin && trip?.status !== "IN_PROGRESS" ? [rider, tripOrigin] : [], [rider?.lat, rider?.lng, tripOrigin?.lat, tripOrigin?.lng, trip?.status]);
  ...
  return <View style={[styles.shell, fill ? styles.shellFill : { height }]}>
    <Map style={StyleSheet.absoluteFill} mapStyle={MAP_STYLE} logo={false} attribution androidView="texture" compass tintColor={theme.panel} onPress={event => { if (!editable || !onMapPress) return; const [lng, lat] = event.nativeEvent.lngLat; onMapPress({ lat, lng, address: "Ubicación seleccionada" }); }} onRegionWillChange={event => { if (event.nativeEvent.userInteraction) setFocused(false); }}>
      ...
      {!searching && pickupGuide.length > 1 ? <GeoJSONSource id="pickup-guide" data={lineFeature(pickupGuide)}><Layer id="pickup-guide-shadow" type="line" paint={{ "line-color": "#9D4EDD", "line-width": 8, "line-opacity": .18, "line-blur": 2 }} layout={{ "line-cap": "round" }} /><Layer id="pickup-guide-dashes" type="line" paint={{ "line-color": "#9D4EDD", "line-width": 3, "line-opacity": .88, "line-dasharray": [1.2, 1.25] }} layout={{ "line-cap": "round", "line-join": "round" }} /></GeoJSONSource> : null}
      ...
      {!searching && rider ? <ViewAnnotation id="rider" lngLat={toLngLat(rider)} anchor="bottom"><Pin kind="rider" text="Moto" pulsing={Boolean(trip)} /></ViewAnnotation> : null}
...
const styles = StyleSheet.create({
  ...
  pin_origin: {}, pin_destination: {}, pin_rider: {}, pin_client: {}, pinPulse: { position: "absolute", top: 1, width: 48, height: 48, borderRadius: 24, borderWidth: 2 }, pinPulse_client: { borderColor: theme.violet, backgroundColor: "rgba(157,78,221,.22)" }, pinPulse_rider: { borderColor: "#176BDE", backgroundColor: "rgba(23,107,222,.20)" },
