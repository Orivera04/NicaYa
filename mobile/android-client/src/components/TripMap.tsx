...
export function TripMap({ trip, origin, destination, currentLocation, route = [], editable = false, onMapPress, onOriginPress, onDestinationPress, label, height = 340, fill = false, searching = false, hideLabel = false }: Props) {
  ...
  return <View style={[styles.shell, fill ? styles.shellFill : { height }]}>
    <Map style={StyleSheet.absoluteFill} mapStyle={MAP_STYLE} logo={false} attribution androidView="texture" compass tintColor={theme.panel} onPress={event => { if (!editable || !onMapPress) return; const [lng, lat] = event.nativeEvent.lngLat; onMapPress({ lat, lng, address: "Ubicación seleccionada" }); }} onRegionWillChange={event => { if (event.nativeEvent.userInteraction) setFocused(false); }}>
      ...
      {!searching && planned.length > 1 ? <GeoJSONSource id="planned-route" data={lineFeature(planned)}><Layer id="planned-route-outline" type="line" paint={{ "line-color": "#1677E8", "line-width": 9, "line-opacity": .95 }} layout={{ "line-cap": "round", "line-join": "round" }} /><Layer id="planned-route-core" type="line" paint={{ "line-color": "#FFFFFF", "line-width": 3, "line-opacity": .96 }} layout={{ "line-cap": "round", "line-join": "round" }} /></GeoJSONSource> : null}
      {!searching && history.length > 1 ? <GeoJSONSource id="travelled-route" data={lineFeature(history)}><Layer id="travelled-route-outline" type="line" paint={{ "line-color": "#1677E8", "line-width": 9, "line-opacity": .82 }} layout={{ "line-cap": "round", "line-join": "round" }} /><Layer id="travelled-route-core" type="line" paint={{ "line-color": "#F8FCFF", "line-width": 3, "line-opacity": .95 }} layout={{ "line-cap": "round", "line-join": "round" }} /></GeoJSONSource> : null}
